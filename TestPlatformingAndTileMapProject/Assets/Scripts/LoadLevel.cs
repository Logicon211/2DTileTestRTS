				...
				if(levelItems.Name == "Tiles") {
					...
					foreach (XmlNode levelTile in levelItems.ChildNodes) {
						if(levelTile.Name == "tile") {
							...
							if (id == 0) {
								GameObject tile = Instantiate(Resources.Load("BlackParticleTile"), new Vector3(transform.position.x +(tileX), transform.position.y +(tileY), 0), transform.rotation) as GameObject;
